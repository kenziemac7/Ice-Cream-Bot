...
function extractMeta(html: string, prop: string): string {
  // handles both name= and property= (Open Graph)
  const patterns = [
    new RegExp(
...
function extractTitle(html: string): string {
  ...
  if (!m) return "";
  // Strip common suffixes like "Shop Name | Yelp" or "Shop Name – City"
  // Only split on pipe and em-dash, NOT regular hyphens (hyphens appear in business names)
  return m[1]
    .trim()
    .split(/\s*[|–]\s*/)[0]
    .trim();
...
function extractNameFromYelpUrl(url: string): string {
  try {
    ...
    if (bizMatch) {
      // Remove trailing city slug (e.g. "-san-francisco", "-sf-2")
...
// Domains that publish lists/articles about shops rather than being shops themselves.
// We skip these — fetching them gives us article content, not a single shop's page.
...
// Returns true when a string looks like a bare domain ("yelp.com", "maps.google.com")
// rather than a real business name.
function looksLikeDomain(s: string): boolean {
  return /^[a-zA-Z0-9-]+\.[a-zA-Z]{2,}$/.test(s.trim());
}

// Only block mass corporate chains — not regional/craft chains like Salt & Straw,
// Humphry Slocombe, or Smitten, which are worth seeking out even with multiple locations.
...
function isCityName(candidate: string, city: string): boolean {
  ...
  const cityLower = city.toLowerCase().replace(/,.*$/, "").trim(); // strip ", CA" etc.
...
function extractShopInfo(
  ...
  // og:title often looks like "Bi-Rite Creamery | Yelp" — take the first segment on pipe only
  const ogTitleName = ogTitle ? ogTitle.split(/\s*\|\s*/)[0].trim() : "";

  // og:site_name is reliable for business names even on location subpages (e.g. /location/san-francisco/)
...
function printShop(shop: ShopInfo, index: number): void {
  console.log();
  console.log(
    `  ${chalk.bold.white(`${index}.`)} ${chalk.bold.yellowBright(shop.name)}`,
  );
...
async function main(): Promise<void> {
  ...
  if (!city) {
    console.error("Usage:   npm start -- <city name>");
    console.error('Example: npm start -- "Austin, TX"');
    process.exit(1);
  }
  ...
  if (!apiKey) {
    console.error(
      "Error: BROWSERBASE_API_KEY environment variable is not set.\n" +
        "       Export it before running: export BROWSERBASE_API_KEY=your_key",
    );
    process.exit(1);
  }
  ...
  console.log(
    chalk.dim(`Found ${results.length} unique results across ${queries.length} searches. Fetching page details...\n`),
  );

  const shops: ShopInfo[] = [];

  for (const result of results) {
    // Skip article/media sites — they list many shops, not individual shop pages
    if (isMediaSite(result.url)) {
      console.log(
        chalk.dim(`  Skipping (media site): ${getHostname(result.url)}`),
      );
      continue;
    }
    ...
    try {
      ...
      // Detect un-followed redirects (e.g. "301 Moved Permanently")
      ...
      // Skip pages that yielded no useful shop data (likely an aggregator or blocked page)
      if (
      ...
      // Deduplicate: same hostname or same normalized name
